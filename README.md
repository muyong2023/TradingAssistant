...
| **RSI 信号** | 09:30–16:00 每 5 分钟检查，**日线**与 **5 分钟线** RSI 触及 30/70 与 20/80 两档时分别推送 |
...
```yaml
rsi:
  oversold: [30, 20]     # 先到 30 报一次，继续跌到 20 再报一次
  overbought: [70, 80]
```

两档**独立去重**：先到 28 报过 30 档后，同日再跌到 18 仍会报 20 档；但在同一档内
来回波动不会重复推送。一次直接从 35 跌到 18 只报最深那档，不会连发两条。

内档（20/80）标记为重要 🔴，外档（30/70）为一般 🟡。

写成单个数值（`oversold: 20`）也兼容，即只有一档。日线与分钟线各自独立走这套档位。
